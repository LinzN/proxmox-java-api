...
import it.corsinvest.proxmoxve.api.MethodType;
import it.corsinvest.proxmoxve.api.ResponseType;
...
/**
 * Proxmox BS Client Base
 */
public class PbsClientBase {

    private String _ticketCSRFPreventionToken;
    private String _ticketPBSAuthCookie;
    ...
    public PbsClientBase(String hostname, int port) {
        _hostname = hostname;
        _port = port;
    }
    ...
    public boolean login(String username, String password) throws JSONException {
        ...
        if (result.isSuccessStatusCode()) {
            ...
            _ticketPBSAuthCookie = result.getResponse().getJSONObject("data").getString("ticket");
            return true;
        } else {
            return false;
        }
    ...
    private void setToken(HttpURLConnection httpCon) {
        if (_ticketCSRFPreventionToken != null) {
            ...
            httpCon.setRequestProperty("Cookie", "PBSAuthCookie=" + _ticketPBSAuthCookie);
        }

        if (_apiToken != null && !_apiToken.isEmpty()) {
            httpCon.setRequestProperty("Authorization", "PBSAPIToken=" + _apiToken);
        }
    }
    ...
    private Result executeAction(String resource, MethodType methodType, Map<String, Object> parameters)
        ...
        try {
            ...
            Logger.getLogger(PbsClientBase.class.getName()).log(Level.SEVERE, null, ex);
        }
